...
public:
    int maximalSquare(vector<vector<char>>& matrix) {
        ...
        // 创建一个dp数组，存储每个位置的最大边长
        vector<vector<int>> dp(m, vector<int>(n, 0));

        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                // 只有在 matrix[i][j] == '1' 时才处理
                if (matrix[i][j] == '1') {
                    if (i == 0 || j == 0) {
                        // 对于第一行或第一列，最大边长只能为 1
                        dp[i][j] = 1;
        ...
        return res * res;  // 返回最大正方形的面积
    }
};
```

关键在于理解matrix[i][j]的含义，其大小是上，左上，右三个方向的最小值加一。取“并集”

## 字典树Trie

字典树是一种树形数据结构，用于高效地存储和检索字符串集合，特别适用于前缀匹配问题。每个节点代表一个字符，路径从根节点到某个节点形成一个字符串。
...
struct TrieNode{
    ...
    string word; // 记录到这里，有没有一个单词
    vector<TrieNode*> next; // 26个字母的子节点
    TrieNode(): next(26, nullptr), word(""){}
}
void insertWords(TrieNode* root, string &word)
{
    TrieNode* node = root;
    for(char c: word){
        int index = c - 'a';
...
void findWords(TrieNode* root, string &word){
    TrieNode* node = root;
    for(char c: word){
        int index = c - 'a';
        if(!node->next[index]){
        ...
        node = node->next[index];
    }// 首先这个单词的最后一个字母要在字典树里面
    if(node->word != ""){
        cout << "Found word: " << node->word << endl;// 其次这个单词需要是一个有效节点
    } else {
        cout << "No such word" << endl;
    }
}
...
public:
    vector<string> findWords(vector<vector<char>>& board, vector<string>& words) {
        ...
        for(int i =0; i<m; i++){
            for(int j = 0; j< n; j++){
                starts[board[i][j]].push_back({i, j});
            }
        }
        vector<string> res;
        ...
        for(auto word : words){
            char start = word[0];
            for(auto &[startx, starty] : starts[start]){
                if(dfs(word, 0, startx, starty, board) && !sett.count(word)){
...
private:
    ...
    bool dfs(string &word, int cur,int x, int y, vector<vector<char>>& board){
        ...
        board[x][y] = '#';
        for(int i = 0; i < 4; i++){
            if(dfs(word, cur+1, x+dir[i][0], y+dir[i][1],board)){
                board[x][y] = temp;
                return true;
            }
        }
...
只用dfs，如果遇到很多重复的查询，会超时

将思路逆转，不去进行单独查询，而是将查询形成一个字典树，然后遍历board的每个值，每个值可以作为以这个字母为起点的单词，用Trie中的路径进行指导，看是否存在这个单词。将时间复杂度控制在O(m*n*4^l) l为字典树的深度
